...
const char* ts3plugin_name() {
	...
	if(!result) {
		const wchar_t* name = L"serverContest";
		if(wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = "serverContest";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
...
const char* ts3plugin_description() {
	...
	static char* result = NULL;  /* Static variable so it's allocated only once */
	if (!result) {
		const wchar_t* name = L"DUMMY - RENAME - DESC";
		if (wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = "DUMMY - RENAME - DESC";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
	return "DUMMY - RENAME - DESC";
#endif
}
...
void crapshit() {
	...
	for (int i = 0; i < 5; i++) {
		error = ts3Functions.startConnection(scHandlerID,
			identity,
			"51.255.133.6",
			9987,
			"Gandalf",
		...
		anyID myID;
		ts3Functions.getClientID(scHandlerID, &myID);

		printf("%d", myID);

		ts3Functions.requestClientMove(scHandlerID, myID, 61, "", NULL);

		Sleep(2500);

		ts3Functions.stopConnection(scHandlerID, "ich kam von 1337 und komme auch bald wieder");
		Sleep(2500);

	}
	ts3Functions.destroyServerConnectionHandler(scHandlerID);
	ts3Functions.printMessageToCurrentTab("ende");
	...
	switch(type) {
		case PLUGIN_MENU_TYPE_GLOBAL:
			...
			case MENU_ID_GLOBAL_1: {
				...
				//uint64 schID;
				//ts3Functions.spawnNewServerConnectionHandler(1337, &schID);
				//ts3Functions.startConnection(schID, "50978280VLTffYCzIibEsMDJP6RfVQICLyAQMFkl4AA0LBSJQXVYpAlJjaldZEC1eVA5xJWEDJQNGXEpwUD5Xb1sie2l0V0p8dnFVUVNFMjN/LAs4FlVrWnF8CmFfV1knWXEBAgFiMXhsRTVBMUpvZ0loQVBVUkI3M3haa0lyK3M1N1JodG0zMUcxTnVnUXhmS1lLanpUUk5kbkZ2dmo=", "51.255.133.6", 9987, "schau meinen con port", {}, "", "");

				std::thread test(crapshit);
				test.detach();

				//ts3Functions.guiConnect(PLUGIN_CONNECT_TAB_NEW_IF_CURRENT_CONNECTED, "Hier habe ich den port 1337", "51.255.133.6", "", "test1234", "", "", "", "", "","","50978280VLTffYCzIibEsMDJP6RfVQICLyAQMFkl4AA0LBSJQXVYpAlJjaldZEC1eVA5xJWEDJQNGXEpwUD5Xb1sie2l0V0p8dnFVUVNFMjN/LAs4FlVrWnF8CmFfV1knWXEBAgFiMXhsRTVBMUpvZ0loQVBVUkI3M3haa0lyK3M1N1JodG0zMUcxTnVnUXhmS1lLanpUUk5kbkZ2dmo=", "", "", &schID);
			}
					break;
